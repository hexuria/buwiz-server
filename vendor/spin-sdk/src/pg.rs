...
impl Connection {
    ...
    pub async fn open(address: impl Into<String>) -> Result<Self, Error> {
        // Blocking `open`/`query` return a complete row-set. The 4.2
        // streaming `*-async` imports add waitables that trap on current
        // Wasmtime when called from the HTTP handler:
        // `waitable cannot be used synchronously while added to a waitable set`.
        let address = address.into();
        let inner = wit::postgres::Connection::open(&address)?;
        Ok(Self(inner))
    }
    ...
    pub async fn query(
        ...
        let rowset = self.0.query(&statement, &params)?;
        Ok(QueryResult::from_row_set(rowset))
    }
...
/// Buffered rows from a blocking [`Connection::query`] call.
pub struct BufferedRows {
    remaining: std::vec::IntoIter<Vec<DbValue>>,
}

impl BufferedRows {
    /// Next row of raw database values, if any remain.
    pub async fn next(&mut self) -> Option<Vec<DbValue>> {
        self.remaining.next()
    }
}
...
impl QueryResult {
    fn from_row_set(rowset: wit::postgres::RowSet) -> Self {
        Self {
            columns: Arc::new(rowset.columns),
            rows: BufferedRows {
                remaining: rowset.rows.into_iter(),
            },
        }
    }
    ...
    pub fn rows(&mut self) -> &mut BufferedRows {
        &mut self.rows
    }

    /// Remaining rows plus column metadata.
    pub fn into_inner(self) -> (Vec<Column>, BufferedRows) {
        ((*self.columns).clone(), self.rows)
    }
}
